...
public class bankJd {
	...
	public static void main(String args[]) throws SQLException {
		...
		log.info("________ATM Log open_______"); 
		int i = 0;
		int t = 1;
		int ls = 1;
		String uname = "";
			...
			try {
				if (check_usr == 0 && check_pass == 0) {
					...
					if (op == 1) {
						...
						if (with > cbalance) {
							i += 1;
							log.warn(cname+" balance is less than requested amount"); 
							throw new MyException("***Insufficient balance***");

						} else {
//						System.out.println("success");
							Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/mss", "root",
									"root");
							PreparedStatement ps = con
									.prepareStatement("UPDATE bank SET  balance = ? WHERE (username = ?)");
							...
							con.close();
							if (rs == 1) {
								log.info(cname+" withdraw success  ");
						...
						if (depo < 500) {
							i += 1;
							log.warn("Insert amount more than 500");
							...
							con.close();
							if (rs == 1) {
								log.info(cname+"Balance Updated Succesfully");
						...
						if (cbalance > m) {
							...
							log.info(cname+"Transferd amount Succesfully to "+Tuser);
							System.out.println("Transfer Successfull");
						}
						Connection con1 = DriverManager.getConnection("jdbc:mysql://localhost:3306/mss", "root",
								"root");
						System.out.println("connection successfully established");
						...
						log.info(cname+"Logged out  Succesfully ");
						break;
					} else {
						i += 1;
						throw new MyException("Invalid option");
					...
					i += 1;
					ls = 1;
					log.warn(uname+" entered invalid credintials"); 
		...
		log.info("_____________________________"); 
		log.info("________ATM log Close_______");
	}
